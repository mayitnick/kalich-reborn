...
user_department = {}          # chat_id -> выбранное отделение (после выбора, ожидание группы)
                ...
                for a in soup.find_all("a", href=True):
                    ...
                    if '?group_id=' in href:
                        try:
                            ...
                            # Очистка названия группы от markdown-символов (если есть)
                            gname = a.get_text(strip=True).replace('*', '').strip()
                            if gname and gname not in ["ПОНЕДЕЛЬНИК", "ВТОРНИК", "СРЕДА", "ЧЕТВЕРГ", "ПЯТНИЦА", "СУББОТА"]:
                                new_cache[gname] = [dep, gid] # Сохраняем как list для JSON
                        except:
                            continue
        except Exception as e:
...
def init_db():
    ...
    conn.execute('CREATE TABLE IF NOT EXISTS item_stickers (chat_id INTEGER, item_key TEXT, sticker_id TEXT, PRIMARY KEY (chat_id, item_key))')
    ...
    if cursor.fetchone():
        ...
        if 'department' not in columns:
            conn.execute('CREATE TABLE schedules_new (group_id INTEGER, day INTEGER, content_hash TEXT, lessons_text TEXT, department INTEGER, PRIMARY KEY (group_id, day, department))')
            conn.execute('INSERT INTO schedules_new (group_id, day, content_hash, lessons_text, department) SELECT group_id, day, content_hash, lessons_text, 3 FROM schedules')
...
def save_item_sticker(chat_id, item_name, sticker_id):
    ...
    conn = sqlite3.connect(DB_FILE)
    conn.execute("INSERT OR REPLACE INTO item_stickers VALUES (?, ?, ?)", (chat_id, key, sticker_id))
    conn.commit()
    conn.close()

def get_item_sticker(chat_id, raw_item_name):
    if not raw_item_name: return None
    clean_name = re.sub(r'\(?\d{2,4}[А-Яа-я]?\)?', '', raw_item_name).strip()
    key = re.sub(r'[^а-яА-Яa-zA-ZёЁ]', '', clean_name).lower()
    conn = sqlite3.connect(DB_FILE)
    res = conn.execute("SELECT sticker_id FROM item_stickers WHERE chat_id=? AND item_key=?", (chat_id, key)).fetchone()
    conn.close()
...
def get_all_schedules_for_day(day):
    ...
    try:
        conn = sqlite3.connect(DB_FILE)
        rows = conn.execute("SELECT department, group_id, lessons_text FROM schedules WHERE day=?", (day,)).fetchall()
        conn.close()
        data = {}
...
def get_schedule_history_for_date(date_str):
    ...
    try:
        conn = sqlite3.connect(DB_FILE)
        rows = conn.execute("SELECT department, group_id, lessons_text FROM schedule_history WHERE date=?", (date_str,)).fetchall()
        conn.close()
        data = {}
        for dep, gid, lessons_json in rows:
            try:
...
class CustomNamesManager:
    ...
    def set_name(self, cid, old, new):
        cid = str(cid)
        if cid not in self.data: self.data[cid] = {}
        key = re.sub(r'[^а-яА-Яa-zA-ZёЁ]', '', old).lower()
        if key: self.data[cid][key] = str(new).strip(); self.save()
...
def get_teacher_info(chat_id):
    ...
    conn = sqlite3.connect(DB_FILE)
    res = conn.execute("SELECT department, rooms FROM teachers WHERE chat_id=? AND status='approved'", (chat_id,)).fetchone()
    conn.close()
    if res:
...
def save_teacher_override(teacher_chat_id, department, day, slot_idx, group_id, new_room, new_subject):
    ...
    if existing:
        ...
        conn.execute(
            ...
            (teacher_chat_id, department, day, slot_idx, group_id, new_room, new_subject, notify_after)
        )
    conn.commit()
    conn.close()
...
def get_status():
    now = datetime.now(); curr = now.strftime("%H:%M"); wd = now.isoweekday()
    if wd > 5: return "rest", None, None
    max_l = 10 if wd == 1 else 8
    last_call = CALLS[max_l-1][1]
    if curr >= last_call: return "rest", None, None
...
def reply_safe(message, text, parse_mode='Markdown'):
    try: bot.send_message(message.chat.id, text, parse_mode=parse_mode, message_thread_id=message.message_thread_id)
    except: pass
...
def create_custom_sticker(text, command, author=None):
    ...
    if author: text = f"«{text}»"
    lines = textwrap.wrap(text, width=15 if command in ['s_fire', 's_blood'] else 18)
    display_text = "\n".join(lines)
    if author: display_text += f"\n\n— {author}"
    ...
    while f_size > 20:
        bbox = draw.multiline_textbbox((0, 0), display_text, font=font, align="center", spacing=10)
        if (bbox[2]-bbox[0]) < size-80 and (bbox[3]-bbox[1]) < size-100: break
        f_size -= 5
        font = get_sticker_font(f_name, f_size)
    ...
    if command == 's_fire':
        draw.multiline_text(pos, display_text, fill="black", font=font, stroke_width=10, align="center", spacing=10)
        draw.multiline_text(pos, display_text, fill="#FF4500", font=font, align="center", spacing=10)
        draw.multiline_text((pos[0], pos[1]-3), display_text, fill="#FFD700", font=font, align="center", spacing=10)
    elif command == 's_blood':
        for offset in range(8, 0, -2):
            draw.multiline_text((pos[0], pos[1] + offset), display_text, fill="#8B0000", font=font, align="center", spacing=10)
        draw.multiline_text(pos, display_text, fill="#FF0000", font=font, stroke_width=2, stroke_fill="black", align="center", spacing=10)
    elif command == 's_glitch':
        draw.multiline_text((pos[0]-4, pos[1]), display_text, fill="#00FFFF", font=font, align="center", spacing=10)
        draw.multiline_text((pos[0]+4, pos[1]), display_text, fill="#FF00FF", font=font, align="center", spacing=10)
        draw.multiline_text(pos, display_text, fill="white", font=font, align="center", spacing=10)
    else:
        draw.multiline_text(pos, display_text, fill="white", font=font, stroke_width=8 if not author else 0, stroke_fill="black", align="center", spacing=10)
...
def get_language_code(text):
    ...
    if clean in LANGUAGES: return clean
...
def process_audio_effects(voice_io, effect=None):
    try:
        ...
        if effect == "chip":
            ...
            echo = song - 10
            song = song.overlay(echo, position=200).overlay(echo - 5, position=400)
        elif effect == "robot":
            combined = song
            for i in range(1, 5):
                delayed = song - (i * 3)
...
def send_updates_for_day(day, data):
    ...
    for m in monitor_manager.active_monitors.values():
        ...
        day_of_week = {1: "понедельник", 2: "вторник", 3: "среда", 4: "четверг", 5: "пятница", 6: "суббота"}.get(day, "неизвестно")
...
def ad_and_execute(message):
    ...
    if cmd == 'r': cmd_r_today(message)
    elif cmd == 'db': cmd_db_router(message)
    elif cmd == 'time': cmd_time(message)
    elif cmd == 'unsub': cmd_unsub(message)
...
def cmd_clear_stickers(message):
    try:
        conn = sqlite3.connect(DB_FILE)
        conn.execute("DELETE FROM item_stickers WHERE chat_id = ?", (message.chat.id,))
        conn.commit()
        conn.close()
        bot.send_message(message.chat.id, "✅ Все привязки стикеров в этом чате удалены.")
        bot.send_message(message.chat.id, "Подписывайтесь на @kalichoctu", message_thread_id=message.message_thread_id)
    except Exception as e:
        reply_safe(message, wrap_code(f"Ошибка при очистке: {e}"))

def _render_schedule_msg(message, monitor, all_data, day, header_text, send_stickers=False):
    key = (monitor['department'], monitor['group_id'])
    lessons = all_data.get(key)
    if not lessons: 
        return reply_safe(message, wrap_code(f"{header_text}\n\nНет пар или данных."))
    ...
    if send_stickers:
        ...
        for l_raw in lessons:
            ...
            if stk and stk not in sent_stickers:
                bot.send_sticker(message.chat.id, stk, message_thread_id=message.message_thread_id)
                sent_stickers.add(stk)
        if not sent_stickers: pass
        
    res = f"{header_text}\n\n"
    cnt = 1
    for i, l in enumerate(lessons):
        lines = format_with_overlap(message.chat.id, monitor['department'], monitor['group_id'], day, i, str(l), all_data)
        if not lines: continue
...
def cmd_r_today(message):
    ...
    day = datetime.now().isoweekday()
    if day > 5: return reply_safe(message, wrap_code("Отдыхай (выходной)"))
    mons = monitor_manager.get_user_monitors(message.chat.id)
    if not mons: return reply_safe(message, "❌ Нет подписок.")
    all_day_data = get_all_schedules_for_day(day)
    all_day_data = apply_teacher_overrides(all_day_data, day)
    for m in mons:
        _render_schedule_msg(message, m, all_day_data, day, f"📅 Сегодня: {m['group_name']}", send_stickers=True)
...
def cmd_db_router(message):
    ...
    if re.match(r'^\d{2}\.\d{2}\.\d{4}$', args):
        try:
            target_date = datetime.strptime(args, "%d.%m.%Y").strftime("%Y-%m-%d")
            all_data = get_schedule_history_for_date(target_date)
            if not all_data: return reply_safe(message, wrap_code(f"🗄 Нет данных в архиве за {args}"))
            
            target_weekday = datetime.strptime(args, "%d.%m.%Y").isoweekday()
            all_data = apply_teacher_overrides(all_data, target_weekday)
            for m in mons:
                header = f"🗄 Архив ({args}): {m['group_name']}"
                _render_schedule_msg(message, m, all_data, target_weekday, header, send_stickers=False)
            return
        except ValueError:
            return reply_safe(message, wrap_code("Неверный формат даты. Используйте ДД.ММ.ГГГГ"))
        ...
        if not all_data:
            ...
            for name, info in GROUP_NAME_TO_ID.items():
                ...
                if dep == mons[0]['department']: 
                    ...
                    if lessons: all_data[(dep, gid)] = lessons

        for m in mons:
            header = f"🗓 Расписание на {day_labels[target_day]}: {m['group_name']}"
            _render_schedule_msg(message, m, all_data, target_day, header, send_stickers=False)
        return
    ...
    for m in mons:
        ...
        _render_schedule_msg(message, m, all_data, next_day, header, send_stickers=True)
...
def cmd_now(message):
    ...
    status, _, idx = get_status()
    if status == "rest" or idx is None: return reply_safe(message, wrap_code("Отдыхай\n(Используй /db)") + "\n\n/db")
    mons = monitor_manager.get_user_monitors(message.chat.id)
    if not mons: return
    day = datetime.now().isoweekday()
    all_data = get_all_schedules_for_day(day)
    ...
    if lessons and len(lessons) > idx:
        ...
        if stk: bot.send_sticker(message.chat.id, stk, message_thread_id=message.message_thread_id)
        else: pass
        def clean_n(t): return re.sub(r'\(?\d{2,4}[А-Яа-я]?\)?', '', str(t)).strip().lower()
        target_n = clean_n(curr_l_raw)
        first_idx = idx
        while first_idx > 0 and clean_n(lessons[first_idx-1]) == target_n: first_idx -= 1
        last_idx = idx
        while last_idx < len(lessons)-1 and clean_n(lessons[last_idx+1]) == target_n: last_idx += 1
        now_dt = datetime.now()
        fmt = "%H:%M"
        ...
        bar = "█" * int(percent // 10) + "░" * (10 - int(percent // 10))
        lines = format_with_overlap(message.chat.id, m['department'], m['group_id'], day, idx, str(curr_l_raw), all_data)
        res_line = lines[0] if lines else str(curr_l_raw)
        td = end_dt - curr_dt
        h, m_rem = td.seconds // 3600, (td.seconds // 60) % 60
        rem = f"{f'{h}ч ' if h > 0 else ''}{m_rem}м"
        lbl = "До конца блока:" if last_idx > first_idx else "До конца пары:"
        reply_safe(message, wrap_code(f"{res_line}\n{bar} {int(percent)}%\n{lbl} {rem}"))
...
def cmd_mem(message):
    ...
    if not sep: return reply_safe(message, wrap_code("Ошибка.\nФормат: /mem Предмет - Замена"))
    try:
    ...
    except: reply_safe(message, wrap_code("Ошибка."))

@bot.message_handler(commands=['setsticker'])
def cmd_setsticker(message):
    item_name = message.text.replace('/setsticker', '').strip()
    if not item_name: return reply_safe(message, "⚠️ Введите название предмета.")
...
def cmd_list(message):
    if is_teacher(message.chat.id):
        dept, rooms = get_teacher_info(message.chat.id)
        return reply_safe(message, f"🧑‍🏫 *Учитель*\nОтделение: {dept}\nКабинеты: {', '.join(rooms)}")
    mons = monitor_manager.get_user_monitors(message.chat.id)
...
def cmd_unsub(message):
    if is_teacher(message.chat.id):
        ...
        conn.commit()
        conn.close()
        return reply_safe(message, "🗑 Учительский профиль удалён. /start — зарегистрироваться снова.")
    to_del = [k for k, v in monitor_manager.active_monitors.items() if str(v['chat_id']) == str(message.chat.id)]
    for k in to_del: del monitor_manager.active_monitors[k]
...
@bot.message_handler(commands=['s', 's1', 's2', 's3', 's_fire', 's_blood', 's_glitch'])
def cmd_text_to_sticker(message):
    ...
    try:
        ...
        bot.send_sticker(message.chat.id, sticker, message_thread_id=message.message_thread_id)
    except Exception as e:
...
def cmd_gs(message):
    ...
    lang = 'ru'
    text = ""
    known_effects = ['chip', 'demon', 'echo', 'robot', 'radio', 'vibe', 'slow', 'fast', 'reverb']
    ...
    try:
        bot.send_chat_action(message.chat.id, 'record_audio', message_thread_id=message.message_thread_id)
...
def cmd_r_voice(message):
    ...
    for m in mons:
        ...
        for i, l in enumerate(lessons):
            lines = format_with_overlap(message.chat.id, m['department'], m['group_id'], day, i, str(l), all_day_data)
            if lines:
        ...
        try:
            ...
            final_voice = process_audio_effects(temp_io, effect=chosen_effect if chosen_effect != 'none' else None)
            bot.send_voice(message.chat.id, final_voice, message_thread_id=message.message_thread_id)
        except Exception as e:
            reply_safe(message, f"❌ Ошибка при генерации голосового расписания: {e}")
...
def handle_clear(message):
    try:
        for _ in range(4):
            bot.send_sticker(message.chat.id, hdxvhgv, message_thread_id=message.message_thread_id)
            time.sleep(0.3)
    except Exception as e:
        print(f"Ошибка: {e}")
...
def handle_teacher_approval(call):
    ...
    if action == 'approve':
        conn = sqlite3.connect(DB_FILE)
        conn.execute("UPDATE teachers SET status='approved' WHERE chat_id=?", (teacher_chat_id,))
        conn.commit()
        conn.close()
        try:
        ...
        conn = sqlite3.connect(DB_FILE)
        conn.execute("DELETE FROM teachers WHERE chat_id=?", (teacher_chat_id,))
        conn.commit()
        conn.close()
        try:
            bot.send_message(teacher_chat_id,
...
def get_user_settings(chat_id):
    try:
        conn = sqlite3.connect(DB_FILE)
        row = conn.execute("SELECT notifications, voice_alerts, fluffy_mode, voice_effect FROM user_settings WHERE chat_id=?", (chat_id,)).fetchone()
        conn.close()
        if row: return {'notifications': row[0], 'voice_alerts': row[1], 'fluffy_mode': row[2], 'voice_effect': row[3] or 'echo'}
    except: pass
...
def set_user_setting(chat_id, key, value):
    try:
        ...
        conn.commit()
        conn.close()
    except Exception as e: logger.error(f"Settings err: {e}")
...
@bot.callback_query_handler(func=lambda c: c.data.startswith('toggle_') or c.data.startswith('cycle_'))
...
def get_next_block_info(cid, department, gid, day, data, current_idx=None):
    try:
        ...
        lines = format_with_overlap(cid, department, gid, day, next_idx, lessons[next_idx], data)
...
def cmd_next(message):
    ...
    mons = monitor_manager.get_user_monitors(message.chat.id)
    if not mons: 
        return reply_safe(message, "❌ Нет подписок. Сначала отправь номер группы.")
    day = datetime.now().isoweekday()
    data = get_all_schedules_for_day(day)
    m = mons[0]
    status, _, idx = get_status()
    info = get_next_block_info(message.chat.id, m['department'], m['group_id'], day, data, idx)
    if info:
...
def format_lessons_count(count):
    if count == 1: return "пол пары"
    if count == 2: return "1 пара"
    if count == 3: return "полторы пары"
    if count == 4: return "2 пары"
...
def cmd_find_by_room(message):
    ...
    if not room_target:
        return reply_safe(message, wrap_code("Ошибка: введите номер кабинета.\nПример: /f 44"))

    mons = monitor_manager.get_user_monitors(message.chat.id)
    if not mons:
        return reply_safe(message, "❌ Нет активных подписок. Сначала подпишитесь на группу, чтобы определить отделение.")
...
def cmd_find_by_group(message):
    ...
    if not target_group: return reply_safe(message, wrap_code("Ошибка: введите группу.\nПример: /w ИС-41-22"))
    ...
    if not group_info: return reply_safe(message, wrap_code(f"Группа {target_group} не найдена."))
    department, gid = group_info[0], group_info[1]
    
    day = datetime.now().isoweekday()
    if day > 5: return reply_safe(message, wrap_code(f"{target_group}: отдых (выходной)"))
    
    all_day_data = get_all_schedules_for_day(day)
    lessons = all_day_data.get((department, gid)) or fetch_lessons(day, gid, department)
    ...
    if lessons:
        ...
        reply_safe(message, wrap_code(f"Нет данных для {target_group} на сегодня."))
        ...
        try:
            ...
            for d in set(days):
                ...
                for name, info in GROUP_NAME_TO_ID.items():
                    ...
                    conn = sqlite3.connect(DB_FILE)
                    old = conn.execute("SELECT content_hash FROM schedules WHERE group_id=? AND day=? AND department=?", (gid, d, dep)).fetchone()
                    conn.close()
...
def process_start_role_selection(chat_id, text, message_thread_id=None):
    if text == '4':
        ...
        bot.send_message(chat_id, "🧑‍🏫 Регистрация учителя.\nВыберите ваше отделение:\n1 - Первое\n2 - Второе\n3 - Третье", message_thread_id=message_thread_id)
    elif text in ('1', '2', '3'):
        ...
        bot.send_message(chat_id, f"Выбрано отделение {dept}. Введите название группы:\n\n" + "\n".join(sorted(groups)), message_thread_id=message_thread_id)
    else:
        bot.send_message(chat_id, "Пожалуйста, выберите роль с помощью кнопок или введите 1, 2, 3 или 4.", message_thread_id=message_thread_id)
...
def handle_start_role(call):
    ...
    process_start_role_selection(chat_id, role_num, call.message.message_thread_id)
    try:
        bot.answer_callback_query(call.id)
...
def handle_all(message):
    ...
    if chat_id in waiting_for_teacher_dept:
        if text in ('1', '2', '3'):
            ...
            bot.send_message(chat_id, "Введите номера ваших кабинетов через запятую.\nПример: 44, 12, 203")
        else:
    ...
    if chat_id in waiting_for_department:
        process_start_role_selection(chat_id, text, getattr(message, 'message_thread_id', None))
        return
    ...
    if chat_id in user_department:
        ...
        for k, v in GROUP_NAME_TO_ID.items():
            if v[0] == dept and (clean_input == k.upper().replace('-', ' ') or input_group == k.upper()):
                found_name = k
                found_gid = v[1]
                break
        ...
        if not found_name:
            for k, v in GROUP_NAME_TO_ID.items():
                if v[0] == dept and (clean_input in k.upper().replace('-', ' ') or input_group in k.upper()):
                    found_name = k
                    found_gid = v[1]
                    break
        ...
        if found_name:
            ...
            groups = [k for k, v in GROUP_NAME_TO_ID.items() if v[0] == dept]
            bot.send_message(chat_id, f"Группа не найдена. Список:\n" + "\n".join(sorted(groups)))
        return
